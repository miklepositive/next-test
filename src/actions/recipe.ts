...
export async function getRecipes() {
    try {
        ...
        return { success: false, error: 'Error while retrieving ingredients' };
    }
}
...
export async function createRecipe(formData: FormData) {
    try {
        ...
        if (!name || ingredients.length === 0) {
            return {
                success: false,
                error: 'Name or at least one ingredient is required',
            };
        }
...
export async function updateRecipe(id: string, formData: FormData) {
    try {
        ...
        return { success: false, error: 'Error while updating recipe' };
    }
}
